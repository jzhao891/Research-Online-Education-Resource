    ...
    def search(query, pages = 1):
        ...
        for i in range(pages):
            ...
            if html:
                ...
                lis = soup.findAll("h3", attrs = { "class" : "r" })
                j = 0
                for li in lis:
                    res = GoogleResult()
                    res.page = i
                    ...
                    url_match = pattern.search(res.link)
...
def get_html(url):
    try:
        ...
        request.add_header("User-Agent", "Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/535.11 (KHTML, like Gecko) Chrome/17.0.963.56 Safari/535.11")
        proxy=urllib2.ProxyHandler({'http':'176.31.190.177:80'})
